@echo off
setlocal
 title KH2FM PS2 BGM MIDI+SF2 Tool
...
if "%~1"=="" (
  echo Drag and drop one or more MIDI files onto this batch.
  echo.
  echo Each MIDI must be next to its matching PS2 files:
  echo   musicXXX.bgm
  echo   waveXXXX.wd
  echo   waveXXXX.sf2
  echo.
  ...
  echo   wave0188.sf2
  echo.
  echo The rebuilt files will be written to an "output" folder next to the MIDI.
  echo.
  pause
  exit /b 1
)
